...
func bytesToKey(b []byte, pasteActive bool) (rune, []byte) {
	...
	// The transport encodes a timed-out standalone Escape as ESC ESC.
	// Partial arrows remain buffered across read boundaries.
	if !pasteActive && len(b) >= 2 && b[0] == keyEscape && b[1] == keyEscape {
...
// SetSize redraws the editable region using its old physical row origin.
// writeLine and writePrompt end full rows with explicit CRLF, not soft wraps.
// Consequently the upstream shrink heuristic (doubling cursorY/maxLine) would
// erase transcript rows above this region. Neither screen clearing nor an
// alternate screen is needed here.
func (t *Terminal) SetSize(width, height int) error {
	...
	if visible {
		...
		t.cursorX = min(t.cursorX, width-1)
		t.clearInput()
	}
	t.termWidth, t.termHeight = width, height
	if visible {
