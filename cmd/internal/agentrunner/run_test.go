...
`), 0o600); err != nil {
	...
	if len(request.Tools) != 6 || !containsTool(request.Tools, "Bash") || !containsTool(request.Tools, "ViewImage") || !containsTool(request.Tools, "SkillUse") || !containsTool(request.Tools, "Read") || !containsTool(request.Tools, "Edit") || !containsTool(request.Tools, "Write") {
		t.Fatalf("tools = %#v, want Bash, ViewImage, SkillUse, Read, Edit, Write", request.Tools)
	}
